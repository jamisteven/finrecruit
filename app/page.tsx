...
export default function HomePage() {
  ...
  return (
    <div style={{ minHeight: '100vh', background: bg, transition: 'background 0.2s', fontFamily: "'Inter', sans-serif" }}>
        ...
        <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '0 24px', height: '52px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            {usingDemo && (
              <span style={{
                ...
                background: accentBg, padding: '2px 6px', borderRadius: '3px',
              }}>Demo</span>
            )}
          </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <button onClick={() => setDarkMode(!dm)} style={{
              width: '32px', height: '32px', borderRadius: '7px', display: 'flex',
      ...
      {/* Hero strip */}
      <div style={{
        background: dm ? '#0D1526' : '#EEF2FF',
        borderBottom: `1px solid ${dm ? '#1a2540' : '#D6DFF8'}`,
        padding: '18px 24px',
      }}>
        <div style={{ maxWidth: '1280px', margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            ...
            <p style={{ fontSize: '12px', color: textSecondary, margin: '3px 0 0' }}>
              Updated twice daily · Finance, Tech, Legal, Marketing · Global
            </p>
          </div>
          <div style={{ display: 'flex', gap: '20px' }}>
            {[
              { label: 'Live jobs', value: displayJobs.length },
              { label: 'Locations', value: availableLocations.length },
      ...
      <div style={{ maxWidth: '1280px', margin: '0 auto', padding: '24px', display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
        ...
        <div ref={sidebarRef} style={{
          ...
          {/* Sector nav */}
          <div style={{ background: surface, borderRadius: '10px', border: `1px solid ${border}`, overflow: 'hidden', marginBottom: '8px' }}>
            <div style={{ padding: '10px 14px 6px', fontSize: '10px', fontWeight: 600, letterSpacing: '0.07em', textTransform: 'uppercase', color: textMuted }}>
              Sector
            </div>
            {SECTORS.map((s) => {
              ...
              return (
                <button key={s.id} onClick={() => setFilters({ ...filters, sector: s.id })} style={{
                  ...
                  {s.label}
                  {!usingDemo && (
                    <span style={{ marginLeft: 'auto', fontSize: '11px', color: textMuted, fontFamily: "'DM Mono', monospace" }}>
                      {s.id === 'all' ? allJobs.length : allJobs.filter(j => j.sector === s.id).length}
                    </span>
                  )}
                </button>
              )
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '12px' }}>
            ...
            {(filters.locations.length > 0 || filters.seniority !== 'All' || filters.search) && (
              <button onClick={() => setFilters(DEFAULT_FILTERS)} style={{
                fontSize: '11px', color: accent, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 500,
              }}>
                Clear filters
              </button>
            )}
          ...
          {loading ? (
            ...
            <div style={{
              ...
              <p style={{ fontSize: '13px', color: textSecondary, marginBottom: '16px' }}>Try adjusting your sector, location, or seniority</p>
              <button onClick={() => setFilters(DEFAULT_FILTERS)} style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
              {displayJobs.map((job, i) => (
                <JobCard key={job.id} job={job} index={i} darkMode={dm} />
              ))}
            </div>
          )}
          ...
          {/* Footer */}
          <div style={{
            marginTop: '32px', paddingTop: '20px', borderTop: `1px solid ${border}`,
