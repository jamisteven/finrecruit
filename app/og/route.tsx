...
export async function GET(req: Request) {
  ...
  return new ImageResponse(
    <div style={{
      ...
      <div style={{ display: 'flex', fontSize: 28, color: '#1A1A1A' }}>
        backchannel.jobs
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', fontSize: 76, fontWeight: 700, color: '#1A1A1A', fontFamily: 'Fraunces', marginTop: 60, lineHeight: 1.1 }}>
        <span>The jobs LinkedIn</span>
        <span style={{ fontStyle: 'italic' }}>{"doesn't show you."}</span>
      </div>
      <div style={{ display: 'flex', fontSize: 22, color: '#6B6560', marginTop: 28 }}>
